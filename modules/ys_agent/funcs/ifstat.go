...
func NetMetrics() []*cc.MetricValue {
  return CoreNetMetrics(nil)
}

func CoreNetMetrics(ifacePrefix []string) []*cc.MetricValue {

  netIfs, err := NetIfs(ifacePrefix)
  if err != nil {
    log.Println(err)
    return nil
  }

  var ret []*cc.MetricValue;
  ...
  ret = append(ret, CounterValue("net.if.in", inBytes))
  ret = append(ret, CounterValue("net.if.out", outBytes))
  ret = append(ret, CounterValue("net.if.total", totalBytes))
  return ret
}
